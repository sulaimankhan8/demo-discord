// tests/run-stateful.js
import { startUser } from "./stateful-user.js";
import fs from "fs";
...
const TOTAL_RUNTIME_MIN = 60;       // ← change here
const REPORT_INTERVAL_MIN = 5;      // ← every 5 minutes

const outputFile = `stateful-report-${Date.now()}.jsonl`;
...
const config = {
  url: "https://demo-discord.onrender.com",
  totalRuntimeMs: TOTAL_RUNTIME_MIN * 60 * 1000,
  reportIntervalMs: REPORT_INTERVAL_MIN * 60 * 1000,
  outputFile,
};

/* ---------------- START ---------------- */

console.log(`Stateful test started`);
console.log(`Runtime: ${TOTAL_RUNTIME_MIN} min`);
console.log(`Snapshot every: ${REPORT_INTERVAL_MIN} min`);
console.log(`Output: ${outputFile}`);

users.forEach((u) => startUser(u, config));
